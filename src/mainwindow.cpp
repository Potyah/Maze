...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
    ...
    try {
        ...
        cells = new Cellular(widthCaves_, heightCaves_, birthLimit_, deathLimit_, probability_);
    ...
    pointCount = 0;
    iteration_ = 0;
    connect(scene, SIGNAL(mouseLeft(double, double)), this, SLOT(firstPointSlot(double, double)));
    linesPainted = false;
}
...
void MainWindow::paintRight(double w_step, double h_step, int i, int j, QPen pen) {
    scene->addLine(w_step * (i + 1), h_step * j, w_step * (i + 1), h_step * (j + 1), pen);
}

void MainWindow::paintDown(double w_step, double h_step, int i, int j, QPen pen) {
    scene->addLine(w_step * (i), h_step * (j + 1), w_step * (i + 1), h_step * (j + 1), pen);
}

void MainWindow::paintWay(double w_step, double h_step, std::vector<std::pair<int, int>> vec, QPen pen) {
...
void MainWindow::paintCells() {
    ...
    for (int i = 0; i < widthCaves_; i++) {
        for (int j = 0; j < heightCaves_; j++) {
            ...
            scene->addRect(w_step * i, h_step * j, w_step * (i + 1), h_step * (j + 1), pen, brush);
        }
    }
}
...
void MainWindow::firstPointSlot(double x, double y) {
    if (linesPainted == true && ui->mazeRadioButton->isChecked()) {
        ...
        if (pointCount == 0) {
            ...
            scene->addLine(w_step * (xx) + w_step / 2, h_step * yy + h_step / 2, w_step * (xx) + w_step / 2,
                           h_step * yy + h_step / 2, pen);
            ++pointCount;
            ...
            if (maze != nullptr) {
                try {
                    std::vector<std::pair<int, int>> rez = maze->findWay(
                        firstPoint.second, firstPoint.first, secondPoint.second, secondPoint.first);
...
void MainWindow::on_loadCavesButton_clicked() {
    QString filepath = QFileDialog::getOpenFileName(this, tr("Open File"), QDir::homePath(), "*.cave");
    if (!filepath.isEmpty()) {
        try {
...
void MainWindow::on_saveCavesButton_clicked() {
    QString filepath = QFileDialog::getSaveFileName(this, tr("Save File"), QDir::homePath(), "*.cave");
    if (!filepath.isEmpty()) {
        try {
...
void MainWindow::on_loadMazeButton_clicked() {
    QString filepath = QFileDialog::getOpenFileName(this, tr("Open File"), QDir::homePath(), "*.maze");
...
void MainWindow::on_saveMazeButton_clicked() {
    QString filepath = QFileDialog::getSaveFileName(this, tr("Save File"), QDir::homePath(), "*.maze");
    if (!filepath.isEmpty() && maze != nullptr) {
        try {
