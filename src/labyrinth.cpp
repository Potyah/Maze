...
void Labyrinth::labyrinthLoader(std::string path) {
    ...
    if (countAllRows < 0 || countAllRows > 50 || lenghtRow < 0 || lenghtRow > 50) {
...
std::vector<std::pair<int, int>> Labyrinth::findWay(int rowNumberStart, int posInRowStart, int rowNumberEnd,
                                                    int posInRowEnd) {
    if (rowNumberEnd > m_countAllRows || rowNumberEnd < 0) {
        throw std::invalid_argument("ERROR on findWay. The point is outside the maze.");
    }
    ...
    bool tmp =
        recursionFind({true, true, true, true}, {rowNumberStart, posInRowStart}, {rowNumberEnd, posInRowEnd});
    if (tmp == false) {
...
bool Labyrinth::recursionFind(std::tuple<bool, bool, bool, bool> side, std::pair<int, int> start,
                              std::pair<int, int> end) {
    bool rez = false;

    if (start.first - 1 >= 0 && m_rows[start.first - 1].getDownSeparator()[start.second] == false &&
        std::get<0>(side) == true) {
        ...
        rez = recursionFind({true, false, true, true}, {start.first - 1, start.second},
                            {end.first, end.second});
    }

    if (start.first + 1 < m_countAllRows && m_rows[start.first].getDownSeparator()[start.second] == false &&
        ...
        rez = recursionFind({false, true, true, true}, {start.first + 1, start.second},
                            {end.first, end.second});
    }

    if (start.second - 1 >= 0 && m_rows[start.first].getRightSeparator()[start.second - 1] == false &&
        ...
        rez = recursionFind({true, true, true, false}, {start.first, start.second - 1},
                            {end.first, end.second});
    }

    if (start.second + 1 < m_lenghtRow && m_rows[start.first].getRightSeparator()[start.second] == false &&
        ...
        rez = recursionFind({true, true, false, true}, {start.first, start.second + 1},
                            {end.first, end.second});
    }
