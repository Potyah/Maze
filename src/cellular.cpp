...
Cellular::Cellular(int width, int height) : width_(width), height_(height), field_(nullptr) {
...
Cellular::Cellular(int width, int height, int lifeLimit, int deathLimit, int probability)
    : width_(width),
      height_(height),
        ...
        for (int j = 0; j < width_; j++) {
            ...
            for (int m = i - 1; m <= i + 1; m++) {
                for (int n = j - 1; n <= j + 1; n++) {
                    if (m < 0 || n < 0 || m >= height_ || n >= width_ || (m == i && n == j)) continue;
...
void Cellular::setLifeLimit(int limit) {
    if (limit < 0 || limit > 7) throw std::invalid_argument("Life limit could be between 0 and 7");
    lifeLimit_ = limit;
}

void Cellular::setDeathLimit(int limit) {
    if (limit < 0 || limit > 7) throw std::invalid_argument("Death limit could be between 0 and 7");
    deathLimit_ = limit;
}
